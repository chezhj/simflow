...
def require_api_key(view_func):
    ...
    One indexed lookup on a SHA-256 digest. Keys are 256 bits of CSPRNG output
    from generate_api_key, so there is nothing for a slow KDF to protect — see
    api_key_digest. The previous PBKDF2 scheme cost ~265 ms per call on the
    production host, paid on every request at 1-2 Hz per flying pilot, which
    capped the server at a couple of concurrent pilots per CPU core.

    There is deliberately no fallback to the old scheme: keys predating the
    cutover were invalidated and regenerated by hand. Re-adding one would put
    the KDF back on the hot path, so test_a_pbkdf2_hash_does_not_authenticate
    ...
    def wrapper(request, *args, **kwargs):
        ...
        if auth.startswith("Bearer "):
            profile = UserProfile.objects.filter(
                api_key_sha256=api_key_digest(auth[7:])
            ).first()
